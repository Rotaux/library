...
public class DeleteStudentPane extends SubSystemSettingPane {
	...
	public void init() {
		...
		Text txtSearch = new Text("请输入要删除的读者编号：");
...
//			int n = JOptionPane.showConfirmDialog(null, "您确定要删除这条读者信息吗？", "警告", JOptionPane.OK_CANCEL_OPTION);
//			if (n == 0) {
			if(CommonClass.confirmDialog("确认对话框","警告信息","您确定要删除这条读者信息吗？") == true) {
				String sql = "delete from readerInfo where ReaderID = '" + tfSearch.getText().trim() + "'";
